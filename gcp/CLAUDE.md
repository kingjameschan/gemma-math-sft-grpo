...
本实例可能是 Spot 实例，随时可能被抢占。必须做到：

1. **安装任何包时**：先检查 `free -h` 确认可用内存，编译型包（如 flash-attn）用 `MAX_JOBS=2` 限制并行度，防止 OOM 导致系统无响应
2. **训练期间**：每 10 分钟用 cron 检查 `nvidia-smi` 和 `free -h`，发现异常及时汇报
3. **长任务用 tmux/nohup**：防止 SSH 断开导致任务丢失
4. **定期保存 checkpoint**：确保被抢占时损失最小
5. **训练前**：`nvidia-smi` 确认 GPU 可用，`df -h` 确认磁盘空间
