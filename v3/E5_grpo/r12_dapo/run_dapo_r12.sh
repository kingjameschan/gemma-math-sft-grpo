#!/usr/bin/env bash
# v3 E5 R12 — DAPO 6/6 via verl, optimized over R11
#
# Diff vs R11:
#   ★ gpu_memory_utilization 0.25 → 0.4 (vllm KV cache 翻倍, gen 加速 ~35%)
#   ★ ppo_micro_batch_size_per_gpu 8 → 16 (FSDP2 单卡放得下, actor 加速 ~20%)
#   ★ ACCUM 48 → 24 (mini_batch=384 不变, 数学等价)
#   ★ trainer.val_before_train=False (跳掉 resume 后 10 min val)
#   ★ trainer.save_freq 4 → 8 (60 ckpt → 30 ckpt, 节省 27 min save)
#   ★ algorithm.filter_groups.max_num_gen_batches 10 → 5 (DS 上限收紧)
#   ★ data.max_prompt_length 256 → 192 (GSM8K p99=130, 256 浪费)
#   ★ optim.weight_decay 0.01 → 0.0 (LoRA 标准 Schulman 2025)
#   ★ optim.lr_warmup_steps 7 → 12 (240 step 长跑更平滑)
#   保持 constant LR (no cosine, per user)
...
GROUP_SIZE=8
NUM_ITER=1
PER_DEVICE=12               # ★ R11=8, R12-orig=16 OOM, retry 中间值
ACCUM=32                    # ★ R11=48, R12-orig=24, retry mini_batch 不变 (12*32=384)
MAX_PROMPT_LEN=192          # ★ R11=256
MAX_RESPONSE_LEN=384
WARMUP=3                    # ★ R11=7, R12-orig=12, retry-C=3 (用户要更短 warmup)
WEIGHT_DECAY=0.0            # ★ R11 隐含 0.01
TOTAL_STEPS=${TOTAL_STEPS:-240}
SAVE_FREQ=${SAVE_FREQ:-1}   # ★ R11=4, R12=1 每 step 保存 (用户要细粒度 240 ckpts)

ENABLE_FILTER_GROUPS=True
FILTER_METRIC=acc
MAX_NUM_GEN_BATCHES=5       # ★ R11=10
...
t_start=$(date +%s)
echo "============================================================="
echo "R12: verl DAPO 6/6 — optimized over R11"
echo "  per_device=$PER_DEVICE × accum=$ACCUM × G=$GROUP_SIZE   mini=$((PER_DEVICE * ACCUM))"
echo "  gpu_mem_util=0.3 (retry-B: per_dev 12)  warmup=$WARMUP  wd=$WEIGHT_DECAY  save_freq=$SAVE_FREQ (★ retry-C: 每 step 都保存)"
echo "  loss_type=token-mean  init=base IT  lr=$LR β=0  μ=$NUM_ITER"
echo "  val_before_train=False  max_prompt=$MAX_PROMPT_LEN  max_resp=$MAX_RESPONSE_LEN"
echo "============================================================="
...
while [ \$((\$(date +%s) - START)) -lt 86400 ]; do
    ...
    aws s3 cp /home/ubuntu/r12_full_master.log $S3_PREFIX/logs/master.log --quiet 2>/dev/null || true
    sleep 60
done
...
docker run --rm --gpus all --shm-size=16g \
    ...
    trainer.project_name=v3_grpo_r12 \
