#!/bin/bash
# R12 host post-save hook: always extract LoRA, keep heavy only for latest.
set -uo pipefail
...
LOG=/home/ubuntu/r12_lora_hook.log
CONVERTER=/home/ubuntu/convert_verl_to_peft.py

mkdir -p $LORA_HOST
echo "[$(date -u)] hook started (R12)" >> $LOG
...
while true; do
    ...
    for s in $steps; do
        SRC=$CKPT_HOST/global_step_$s
        DST=$LORA_HOST/global_step_$s
        SRC_PT=$SRC/actor/model_world_size_1_rank_0.pt

        if [ -f "$DST/adapter_model.safetensors" ]; then
            if [ "$s" != "$latest" ] && [ -f "$SRC_PT" ]; then
                echo "[$(date -u)] cleanup heavy for non-latest step $s" >> $LOG
                rm -f $SRC/actor/model_world_size_*.pt
                rm -f $SRC/actor/optim_world_size_*.pt
                rm -f $SRC/actor/extra_state_world_size_*.pt
        ...
        echo "[$(date -u)] extracting LoRA for global_step_$s ($([ "$s" = "$latest" ] && echo "LATEST" || echo "non-latest"))" >> $LOG
        docker run --rm --shm-size=2g \
          ...
          --workdir /workspace \
          --entrypoint python3 $DOCKER_IMG \
          /workspace/convert.py \
          --ckpt /workspace/ckpt_in/global_step_$s \
          --out /workspace/ckpt_out/global_step_$s \
          --base-model /workspace/models/gemma-2-2b-it \
          >> $LOG 2>&1

        if [ -f "$DST/adapter_model.safetensors" ]; then
            echo "[$(date -u)] uploading LoRA $s to S3 lora_only/" >> $LOG
            ...
            if [ "$s" != "$latest" ]; then
                ...
                echo "[$(date -u)] step $s done (LoRA up, heavy KEPT for resume)" >> $LOG
            fi
        else
            echo "[$(date -u)] CONVERT FAILED for step $s" >> $LOG
        fi
    done
    sleep 60
done
