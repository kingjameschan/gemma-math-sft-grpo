#!/bin/bash
# R13 host LoRA hook = R12 hook with RUN_TAG bumped
set -uo pipefail
...
while true; do
    ...
    for s in $steps; do
        ...
        echo "[$(date -u)] extracting LoRA step $s ($([ "$s" = "$latest" ] && echo "LATEST" || echo "non-latest"))" >> $LOG
