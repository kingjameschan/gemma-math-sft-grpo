#!/usr/bin/env bash
# v3 E5 R13 — = R12 retry-C, ONLY reward function changed:
#   R12: Gemini judge (0-10 partial credit) → reward 0-1
#   R13: rule-based binary (extract_answer_strict layers 1-4 + math_equal_numerical)
#        + per-call JSONL log to /home/ubuntu/r13_reward_log.jsonl
#
# All other config IDENTICAL to R12 retry-C:
#   per_dev=12, accum=32, mini_batch=384, G=8
#   gpu_mem=0.3, max_prompt=192, max_resp=384
#   lr=1e-5, warmup=3, μ=1, β=0
#   clip 0.2/0.28, loss_agg=token-mean
#   DS enable, save_freq=1, val_before_train=False, wd=0
set -uo pipefail

V3_HOST=/home/ubuntu/v3
MODELS_HOST=/home/ubuntu/models
RUN_TAG=baseit_r13_verl_dapo_full_15ep
...
REWARD_FN=/workspace/v3/E5_grpo/r13_dapo/reward_judge_r13.py    # ★ R13 only change
...
: > /home/ubuntu/r13_reward_log.jsonl
...
t_start=$(date +%s)
echo "============================================================="
echo "R13: = R12 retry-C, only reward changed (rule-based binary)"
echo "  per_device=$PER_DEVICE × accum=$ACCUM × G=$GROUP_SIZE   mini=$((PER_DEVICE * ACCUM))"
echo "  gpu_mem_util=0.3  warmup=$WARMUP  wd=$WEIGHT_DECAY  save_freq=$SAVE_FREQ"
echo "  reward: $REWARD_FN (rule-based 4-layer extract, no judge LLM)"
echo "  log per-call → /home/ubuntu/r13_reward_log.jsonl"
echo "============================================================="
...
while [ \$((\$(date +%s) - START)) -lt 86400 ]; do
    ...
    aws s3 cp /home/ubuntu/r13_full_master.log $S3_PREFIX/logs/master.log --quiet 2>/dev/null || true
    aws s3 cp /home/ubuntu/r13_reward_log.jsonl $S3_PREFIX/logs/reward_log.jsonl --quiet 2>/dev/null || true
    sleep 60
done
...
# Note: no GOOGLE_API_KEY needed (rule-based, no judge LLM)
docker run --rm --gpus all --shm-size=16g \
    ...
    -e "R13_REWARD_LOG=/workspace/r13_reward_log.jsonl" \
    -v "$V3_HOST":/workspace/v3 \
    -v "$MODELS_HOST":/workspace/models \
    -v "$CKPT_BASE":/workspace/ckpt_out \
    -v "/home/ubuntu/r13_reward_log.jsonl":/workspace/r13_reward_log.jsonl \
    ...
    trainer.project_name=v3_grpo_r13 \
...
aws s3 cp /home/ubuntu/r13_reward_log.jsonl $S3_PREFIX/logs/reward_log.jsonl --quiet || true
