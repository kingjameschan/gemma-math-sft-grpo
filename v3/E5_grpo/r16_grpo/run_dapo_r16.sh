...
#       data=train_gsm8k_math_numerical.parquet (11875), reward=R16 frac-aware
set -uo pipefail

V3_HOST=/home/ubuntu/v3
MODELS_HOST=/home/ubuntu/models
RUN_TAG=baseit_r16_verl_grpo_full_15ep
...
REWARD_FN=/workspace/v3/E5_grpo/r16_grpo/reward_judge_r16.py
MODEL_PATH=/workspace/models/gemma-2-2b-it
S3_PREFIX=s3://kingjameschan-fine-tuning-v3/$RUN_TAG

: > /home/ubuntu/r16_reward_log.jsonl
...
PPO_MINI_BATCH=192                  # ★ R16: 384 → 192
MAX_PROMPT_LEN=192
MAX_RESPONSE_LEN=384  # ★ R16 retry: 512→384 (R15 same OOM pattern, see memory feedback_vllm_wake_up_oom.md)                # ★ R16: 384 → 512
...
t_start=$(date +%s)
echo "============================================================="
echo "R16: R14 + 3 changes (mini 384→192, max_resp 384→512, warmup 3→10)"
...
while [ \$((\$(date +%s) - START)) -lt 86400 ]; do
    ...
    aws s3 cp /home/ubuntu/r16_full_master.log $S3_PREFIX/logs/master.log --quiet 2>/dev/null || true
    aws s3 cp /home/ubuntu/r16_reward_log.jsonl $S3_PREFIX/logs/reward_log.jsonl --quiet 2>/dev/null || true
    sleep 60
done
...
docker run --rm --gpus all --shm-size=16g \
    ...
    -e "R16_REWARD_LOG=/workspace/r16_reward_log.jsonl" \
    -v "$V3_HOST":/workspace/v3 \
    -v "$MODELS_HOST":/workspace/models \
    -v "$CKPT_BASE":/workspace/ckpt_out \
    -v "/home/ubuntu/r16_reward_log.jsonl":/workspace/r16_reward_log.jsonl \
...
aws s3 cp /home/ubuntu/r16_reward_log.jsonl $S3_PREFIX/logs/reward_log.jsonl --quiet || true
